...
class AlternatePayment extends CI_Controller
{
  ...
  // Log cookie data
    ...
    public function insert_order()
    {
        ...
        // If user is not logged in, create user and address records
        // if (empty($user_id)) {
            // Create user record
            $user_data = [
    ...
    //   file_put_contents(APPPATH . 'logs/user_insertion_debug.log', "Attempting to insert user: " . print_r($user_data, true), FILE_APPEND);
        $user_id = $this->AlternatePayment_model->insert_user_data('user_table', $user_data);
// file_put_contents(APPPATH . 'logs/user_insertion_debug.log', "Insert result: " . var_export($user_id, true), FILE_APPEND);
        ...
        foreach ($cart_items as $item) {
            ...
            $order_data = [
                ...
                'product_id'       => $item['product_id'],
                'order_id'         => $merchantOrderId,
                'address_id'      => null, // Not using address IDs anymore
                'product_price'   => $product_price,
                'product_quantity' => $quantity,
                'total_price'     => $product_price * $quantity,
                'created_at'      => date('Y-m-d H:i:s'),
